...
class App {
  ...
  constructor() {
    ...
    containerWorkouts.addEventListener("click", this._reset.bind(this));
  }
  ...
  _newWorkout(e) {
    ...
    if (type === "running") {
      ...
      if (
        ...
        return alert("wrongggggg!!!!!");
      workout = new Running([lat, lng], distance, duration, cadence);
    }
    if (type === "cycling") {
      const elevation = +inputElevation.value;
      if (!validNumber(distance, duration) || !posNumber(distance, duration))
        return alert("wrongggggg!!!!!");
  ...
  _moveToPopup(e) {
    const worklEl = e.target.closest(".workout");

    if (!worklEl) return;

    const workout = this.#workouts.find(
      (work) => work.id === worklEl.dataset.id
    );
  ...
  _reset(e) {
    const tar = e.target.closest(".close");
    if (!tar) return;
    const workoutEl = tar.closest(".workout");
    workoutEl.remove();
